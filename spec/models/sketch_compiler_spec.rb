...
context "SketchCompiler.new" do
  ...
  it "should correctly absolutize a path with /../ that starts at /" do
    SketchCompiler.new(@as).path.should == FileUtils.pwd() + "/lib/examples/add_hysteresis.rb"
  end
  
  it "should correct absolutize a relative path" do
    SketchCompiler.new("lib/examples/add_hysteresis.rb").path.should == FileUtils.pwd() + "/lib/examples/add_hysteresis.rb"
  end
...
context "SketchCompiler#create_build_dir! without a path prefix" do
  ...
  it "should create the sketch dir in the correct place" do
    @sc.should_receive( :mkdir_p ).with( FileUtils.pwd() + "/lib/examples/add_hysteresis" )
    @sc.create_build_dir!
  end
end
...
context "SketchCompiler#build_dir" do
  ...
  it "should be correct with a default target_dir" do
    @sc.build_dir.should == FileUtils.pwd() + "/lib/examples/add_hysteresis"
  end
